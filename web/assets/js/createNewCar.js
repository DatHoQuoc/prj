...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (modelSelect) {
        modelSelect.addEventListener('change', () => {
            generateSerial(modelSelect);
        });

    } else {
        console.log("One or more form elements not found. Check that your IDs match the HTML elements.");
    }

    generateSerial(modelSelect);
});

function generateSerial(modelSelect) {
    const model = modelSelect.value;
    if (model) {
        const modelCode = model.substring(0, 3).toUpperCase();
        const randomNumber = Math.random().toString(2).substring(2, 4) + Date.now();
        const serialNumber = `${modelCode}${randomNumber}`;
        document.getElementById('carSerialNumber').textContent = serialNumber;
        document.getElementById('carSerialNumberInput').value = serialNumber;
    }
